...
def compute_feature_importance(model,X_train):
    ...
    feature_importance = pd.DataFrame({'Feature': X_train.columns, 'Importance': np.abs(importance)})
...
def plot_roc_curve(roc_out, img_filename):
    ...
    fpr, tpr, thresholds = metrics.roc_curve(roc_out['Churn'], roc_out['logisticregression_predict_1'])
...
def evaluate(context: ModelContext, **kwargs):
    ...
    LR_classifier = osml.load(model_name="LR_classifier")
    predict_df =LR_classifier.predict(X_test,y_test)
    accuracy_rf = LR_classifier.score(X_test, y_test)
    df = X_test.sample(n=1)
    df = df.drop(columns="sampleid")
   
    
    # Initialize the SHAP explainer
    explainer = shap.Explainer(LR_classifier.modelObj, X_test.to_pandas())
    shap_values_test = explainer(X_test.to_pandas())
    
    shap.summary_plot(shap_values_test, X_test.to_pandas(),show=False, plot_size=(12, 8), plot_type='bar')
    save_plot('SHAP Feature Importance', context=context)
    
    
    shap.plots.beeswarm(shap_values_test,show=False, plot_size=(12,8))
    ...
    ClassificationEvaluator_obj = ClassificationEvaluator(
        ...
        prediction_column='logisticregression_predict_1',
        num_labels=2
    )
...
#      # Extract and store evaluation metrics
    ...
    cm = confusion_matrix(predict_df.to_pandas()['Churn'], predict_df.to_pandas()['logisticregression_predict_1'])
    plot_confusion_matrix(cm, f"{context.artifact_output_path}/confusion_matrix")
    
    # Generate and save ROC curve plot
    roc_out = ROC(
        data=predict_df,
        probability_column='logisticregression_predict_1',
        observation_column=target_name,
        positive_class='1',
        num_thresholds=1000
    )
    ...
    feature_importance = compute_feature_importance(LR_classifier.modelObj,X_test)
